...
import { prepareVault, verifySecurityAnswerHash, encryptMetadata, hashSecurityQuestionAnswers, decryptMetadata } from "../services/vault-service.js";
...
const clientEncryptedSchema = z.object({
  ...
  metadata: z.object({
    ...
    willType: z.string(),
    fractionKeyCommitments: z.any().optional(),
...
vaultRouter.post("/:vaultId/prepare-client", async (req, res, next) => {
  try {
    ...
    if (parsed.metadata.isPqcEnabled) {
       // Allow PQC in frontend-only encryption mode now
    }
